...
If `L` is separably closed (e.g. algebraically closed) and `p` is a prime
...
* `ModularCyclotomicCharacter n : L ≃+* L →* ZMod d` sends `g` to the unique `j` such that `g(ζ)=ζ^j` for
    all `ζ : rootsOfUnity n L`.

## Todo

Glue the `ModularCyclotomicCharacter`s together. Define the cyclotomic character.
...
example (n : ℕ) [NeZero n] : 0 < n := by exact Fin.size_positive'
...
lemma comp (g h : L ≃+* L) : χ n (g * h) = 
  ...
  congr 2
  simp only [pow_eq_pow_iff_modEq, ← ZMod.nat_cast_eq_nat_cast_iff, SubmonoidClass.coe_pow, ZMod.nat_cast_val, Nat.cast_mul]
  apply ZMod.cast_mul (m := orderOf ζ) orderOf_dvd_card_univ
