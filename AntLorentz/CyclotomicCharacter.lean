...
lemma _root_.rootsOfUnity.card_dvd_of_dvd {a b : ℕ+} (hab : a ∣ b) :
    Fintype.card (rootsOfUnity a L) ∣ Fintype.card (rootsOfUnity b L) := sorry

variable {L}

lemma foobar (a b : ℕ+) (hab : a ∣ b) (g : L ≃+* L) :
    ZMod.castHom (rootsOfUnity.card_dvd_of_dvd L hab) (ZMod (Fintype.card (rootsOfUnity a L))) 
    (ModularCyclotomicCharacter b g) = ModularCyclotomicCharacter a g := sorry
